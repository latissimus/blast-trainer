 ...
 * uebereinander.
 */
export function kurveSvg(reihen, { hoehe = 130, einheit = '' } = {}) {
  ...
  const B_ = 300;
  const zeit = (d) => new Date(d + 'T12:00:00').getTime();
  const t0 = Math.min(...alle.map((p) => zeit(p.datum)));
  const t1 = Math.max(...alle.map((p) => zeit(p.datum)));
  ...
  const x = (d) => B.links + (t1 === t0 ? (B_ - B.links - B.rechts) / 2
    : ((zeit(d) - t0) / (t1 - t0)) * (B_ - B.links - B.rechts));
  const y = (w) => B.oben + (1 - (w - min) / (max - min)) * (hoehe - B.oben - B.unten);

  const linien = reihen.filter((r) => r.werte.length >= 2).map((r) => {
    const pts = r.werte.map((p) => ({ x: x(p.datum), y: y(p.wert) }));
  ...
  const dat = (t) => new Date(t).toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit' });
